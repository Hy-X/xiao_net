...
# Only select events with magnitude above 3
mask = data.metadata["source_magnitude"] > 3
data.filter(mask)

# Only select events with magnitude below 4
mask = data.metadata["source_magnitude"] < 4
data.filter(mask)
...
for i in range(n_samples):
    ...
    # Find peaks in the ground truth labels
    y_p_peaks, _ = find_peaks(sample["y"][0], height=height, distance=100)
    y_s_peaks, _ = find_peaks(sample["y"][1], height=height, distance=100)
