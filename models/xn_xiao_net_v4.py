...
class XiaoNetFast(nn.Module):
    """
    Faster XiaoNet variant with reduced bottleneck width.

    Changes vs original XiaoNet:
    - ConvTranspose1d -> Upsample + Conv1d
    - Removed BatchNorm from decoder
    - Bottleneck channels reduced: *8 -> *6
    """
    ...
    def __init__(self, window_len=1000, in_channels=3, num_phases=3, base_channels=6):
        ...
        # Bottleneck reduced from *8 to *6
        self.bottleneck = self._depthwise_block(
            base_channels * 4, base_channels * 6, stride=4
        )

        # -------- Decoder --------
        self.dec3 = self._upsample_block(base_channels * 6, base_channels * 4, scale=4)
        self.dec2 = self._upsample_block(base_channels * 4, base_channels * 2, scale=4)
        self.dec1 = self._upsample_block(base_channels * 2, base_channels, scale=4)
    ...
    def _upsample_block(self, in_ch, out_ch, scale=4):
        """Nearest upsample + Conv1d (faster than ConvTranspose1d)"""
        return nn.Sequential(
